...
function createGraph(){
    var width = 960; // char width
    var heigth = 700; // char height
    var format = d3.format(",d"); // convert value to integer
    var color = d3.scale.category20(); // create ordinal scale with 20 colors
    var sizeOfRadius = d3.scale.pow().domain([-100,100]).range([-50,50]); // https://github.com/mbostock/d3/wiki/Quantitative-Scale#pow
    var bubble = d3.layout.pack()
      .sort(null) // disable sorting, use DCOM tree traversal
